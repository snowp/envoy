...
public:
   ...
   * @param immediate bool whether the endpoint should be removed immediately, regardless of whether
   * it still exists in service discovery.
   */
  virtual void setUnhealthy(bool immediate) PURE;

  /**
   * Notify the monitor that this host should no longer be considered for immediate failure. This
   * makes it possible to clear the flag in cases where the service discovery data remains the same
   * as an upstream is restarted/redeployed.
   */
