...
void HostSetImpl::updateHosts(UpdateHostsParams&& update_hosts_params,
  ...
  rebuildLocalityScheduler(
      locality_scheduler_, locality_entries_,
      std::bind(&HostSetImpl::effectiveLocalityWeight, this, _1, healthy_hosts_per_locality_),
      *healthy_hosts_);
  rebuildLocalityScheduler(
      degraded_locality_scheduler_, degraded_locality_entries_,
      std::bind(&HostSetImpl::effectiveLocalityWeight, this, _1, degraded_hosts_per_locality_),
      *degraded_hosts_);
...
void HostSetImpl::rebuildLocalityScheduler(
    ...
    std::function<double(uint32_t)> effectiveWeight, const HostVector& hosts) {
  ...
  if (hosts_per_locality_ != nullptr && locality_weights_ != nullptr &&
    ...
    for (uint32_t i = 0; i < hosts_per_locality_->get().size(); ++i) {
      const double effective_weight = effectiveWeight(i);
...
double HostSetImpl::effectiveLocalityWeight(uint32_t index,
                                            HostsPerLocalityConstSharedPtr eligible_hosts) const {
