...
TEST_F(GrpcHttp1BridgeFilterTest, NotGrpc) {
  Http::TestHeaderMapImpl request_headers{{"content-type", "text/html"},
                                          {":path", "/foo"}};

  EXPECT_EQ(Http::FilterHeadersStatus::Continue, filter_.decodeHeaders(request_headers, false));

  // ensure we don't rewrite content-type
  EXPECT_EQ(request_headers.ContentType().value().getStringView(), "text/html");

  Http::TestHeaderMapImpl response_headers{{"content-type", "text/html"}}
  EXPECT_EQ(Http::FilterHeaderStatus::Continue, filter_.encodeHeaders(response_headers, false)
  EXPECT_EQ(response_headers.ContentType().value().getStringView(), "text/html");
